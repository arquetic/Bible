...
def get_verses(soup):
    verses = {}
    statement = soup.find('p', class_='Enunciado')
    if statement:
        verses['statement'] = html.unescape(statement.get_text())
    ...
    if verse_tags:
        for tag in verse_tags:
            ...
            if len(sspp):
                ...
                verses[f"verse_{verse_number}"] = unscape
    return verses
            ...
            if third_ul:
                for li_3 in third_ul.find_all('li'):
                    for a in li_3.find_all('a'):
                        ...
                        verses_soup = get_soup_from_url(verses_url)
                        verses = get_verses(verses_soup)
                        bible[library][book_title][f"chapter_{chapter}"] = {
                            'chapter': chapter,
...
output_file = 'Vatican-Bible.spa.json'
